    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="handleEdit(scope.$index, scope.row)" v-if="scope.row.isSend!=1">确认寄出</el-button>
          <span
            size="mini"
            v-if="scope.row.isSend==1">已寄出
          </span>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export function sendBook(form){
    return request({
      url: '/zuul/bookCross/sendBook',
      headers: {
    ...
    export default {
      ...
      methods:{
        ...
        handleEdit(index, row) {
          ...
          sendBook(this.bookCross).then(response=>{
            this.shuaxin()
